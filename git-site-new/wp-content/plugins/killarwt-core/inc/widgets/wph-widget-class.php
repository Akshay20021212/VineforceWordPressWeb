...
if ( !class_exists( 'WPH_Widget' ) && class_exists( 'WP_Widget' ) ) {
	...
	class WPH_Widget extends WP_Widget {
		...
		function create_field( $key, $out = "" ) {
			...
			if ( !empty(  $key['param_name'] ) && $key['param_name'] == 'category' ) {
				...
				$product_categories_dropdown = array(
					esc_html__( 'Select Category', 'killarwt-core' ) => '',
				);
			...
			$key[ 'type' ] = $this->killarwt_change_field_type( $key[ 'type' ] );
			
			/* Set field class */
			$key['class'] = ( !empty( $key['class'] ) ) ? $key['class'].' widefat' : 'widefat';
			$key['class'] .= $this->killarwt_field_class ( $key[ 'type' ] );
			...
			/* Check for <p> Class */
			$p = ( isset( $key[ 'class-p' ] ) ) ? '<p class="' . $key[ 'class-p' ] . '" ' . killarwt_stringify_atts($p_data) . ' >' : '<p ' . killarwt_stringify_atts($p_data) . '>';
		...
		function killarwt_change_field_type ( $type ) {
		...
		function killarwt_field_class ( $type ) {

			return ' killar-widget-action killar-widget-'.$type;
		}
        ...
        */
        ...
        function create_field_attach_image( $key, $out = "" )
        {
            ...
            $out .= '<div class="media-widget-control"><div class="media-widget-preview"><img src="'.$url.'" class="killar-image-src attachment-thumb" style="'.$style.'" /></div></div>';

            $out .= '<input type="hidden" ';

            if ( isset( $key['class'] ) )
                $out .= 'class="killar-image-upload ' . esc_attr( $key['class'] ) . '" ';
            ...
            $out .= '<button class="button killar-image-upload-btn">' .__('Upload image', 'killarwt-core') . '</button>';
		...
		function create_field_colorpicker( $key, $out = "" ) {
			...
			$key[ 'class' ] = ( !empty( $key[ 'class' ] ) ) ? 'killar-color-picker ' . $key[ 'class' ] : 'killar-color-picker';
			...
			$out .= '<script>jQuery(document).ready( function() {
				jQuery(".killar-color-picker").wpColorPicker();
			} );</script>';
		...
		function create_field_checkbox( $key, $out = "" ) {
			...
			/*if ( $key['param_name'] == 'carousel_nav' ) {
