...
if ( ! function_exists( 'killarwt_features_box' ) ) :
	...
	function killarwt_features_box( $atts = array(), $content = null ) {
		...
		$css_output							= '';
		$args								= $atts;
		$args['id']  						= killarwt_uniqid('killar-features-box-');
		...
		$args['wrap_atts']['class'][] 		= 'killar-features-box';
		...
		$data_attr['class'] = array( 'kwt-features-box', 'kwt-bxim', 'fbox-3' );
		...
		if( $icon_type != 'none' ) {
			...
			$icon_image_wrap_attr['class'][] = ( !in_array( $icon_alignment, array( '' ) ) ) ? 'justify-content-' . $icon_alignment  : '';
			$icon_image_wrap_attr['class'][] = ( !in_array( $icon_alignment_tablet, array( '' ) ) ) ? 'justify-content-md-' . $icon_alignment_tablet : '';
			$icon_image_wrap_attr['class'][] = ( !in_array( $icon_alignment_mobile, array( '' ) ) ) ? 'justify-content-sm-' . $icon_alignment_mobile : '';
			$icon_image_wrap_attr['class'][] = ( $icon_ver_alignment ) ? 'align-items-' . $icon_ver_alignment : '';
			...
			$args['sec_content'] .= '<div ' . killarwt_stringify_atts( $icon_image_wrap_attr ) . '>';
						
				$icon_attr = array();
				$icon_attr['class'] = array( 'fbx-icon', 'bx-ico-img' );
				$icon_attr['class'][] = ( !in_array( $icon_color, array( '', 'custom', 'default' ) ) ) ? 'text-' . $icon_color : '';
				...
				$args['sec_content'] .= '<div ' . killarwt_stringify_atts( $icon_attr ) . '>';
				
				// Icon ---------------------------------------
				
				if( $icon_type == 'icon' && !empty( $atts['icon'] ) ) {
					$args['sec_content'] .= killarwt_elementor_render_icon( $atts['icon'], array( 'class' => 'list-icon' ), 'span' );
				}
				...
				if( $icon_type == 'image' && !empty( $image ) ) {
					...
					$args['sec_content'] .= ( !empty( $image['id'] ) ) ? killarwt_get_image_html( array( 'attach_id' => $image['id'], 'size' => $image_size ) ) :  '<img src="' . $image['url'] . '" />';
					...
					$css_output .= killarwt_output_css( array( '#'. $args['id'] . ' .bx-img' => $bx_img_css_ar ) );			
				}
			...
			if ( in_array( $icon_type, array( 'flat-icon', 'icon', 'text', 'number' ) ) ) {
				...
				$css_output .= killarwt_output_css( array( '#'. $args['id'] . ' .fbx-icon' => $icon_css_ar ) );
			}
			...
			if ( in_array( $icon_type, array( 'image' ) ) ) {
				...
				$css_output .= killarwt_output_css( array( '#'. $args['id'] . ' .fbx-icon' => $image_css_ar ) );
			}
		}
		...
		$html = '';
		ob_start();
		killarwt_exts_get_template( 'shortcodes/features-box', $args );
		$html = ob_get_clean();
