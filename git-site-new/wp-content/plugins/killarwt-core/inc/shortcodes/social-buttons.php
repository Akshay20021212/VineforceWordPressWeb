...
if ( ! function_exists( 'social_buttons' ) ) :
	...
	function social_buttons( $atts = array() ) {
		...
		$atts = shortcode_atts(array(
			...
			'title'							=> '',
			'social_type'					=> killarwt_social_type(),
			'social_display_type' 			=> killarwt_social_links_display_type(),
			'social_icon_style' 			=> killarwt_social_links_icon_style(),
			'social_icon_shape'  	   		=> killarwt_social_links_icon_shape(),
			'social_icon_size'  	   		=> killarwt_social_links_icon_size(),
			'social_links' 	 		  		=> array(),
			'social_share_links' 	 		=> array(),
			'social_profiles_links' 	 	=> array(),
			'social_links_data' 	   		=> killarwt_social_links_data(),
			'social_icon_settings' 	   		=> killarwt_social_links_settings(),
			'el_classes' 					=> '',
			'item_el_classes' 				=> '',
			'wrap_id' 						=> '',
			'wrap_el_classes' 				=> '',
		...
		if ( $atts['social_type'] == 'share' ) {
			$atts['social_display_type'] 	= killarwt_social_share_links_display_type();
			$atts['social_icon_style'] 		= killarwt_social_share_links_icon_style();
			$atts['social_icon_shape'] 		= killarwt_social_share_links_icon_shape();
			$atts['social_icon_size'] 		= killarwt_social_share_links_icon_size();
			$atts['social_links_data'] 		= killarwt_social_share_links_data();
			$atts['social_icon_settings'] 	= killarwt_social_share_links_settings();
		}
		...
		$css_output					= '';
		$args						= $atts;
		$args['id']  				= killarwt_uniqid('killar-social-buttons-');
		...
		$args['wrap_atts']['class'][] 		= 'killar-social-buttons';
		$args['wrap_atts']['class'][] 		= 'kwt-social-btns';
			...
			if ( !empty( $social_links ) ) {
				...
				if ( !empty( $social_links_loop ) ) {
					...
					foreach ( $social_links_loop as $icon_name => $icon_det ) {
						...
						$args['sec_content'] .= '<a href="'.esc_attr( $href ).'" class="social-link social-'.$icon_name.' ' . $item_el_classes . '" title = "'.esc_attr( $icon_det['label'] ).'" data-bs-toggle="tooltip" aria-label="'.esc_attr( $icon_det['label'] ).'" data-bs-original-title="'.esc_attr( $icon_det['label'] ).'" target="_blank">';
						
						if ( in_array( $social_display_type, array( 'default', 'icon-label' ) ) ) {
							$args['sec_content'] .= '<span class="icn"><i class="'.esc_attr( $icon_det['icon_class'] ).'"></i></span>';
						}
		...
		$html = '';
		ob_start();
		killarwt_exts_get_template( 'shortcodes/social-buttons', $args );
		$html = ob_get_clean();
...
add_shortcode( 'killar_social_buttons', 'social_buttons' );