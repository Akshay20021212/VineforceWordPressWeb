var wtadmin = wtadmin || {}, killarMediaInit;
(function ($) {
	"use strict";
	wtadmin.initialization = {
		init: function () {
			this.colorPicker('.killar-color-picker');
			this.activationForm();
			this.pluginManager();
		...
		activationForm : function () {
			$body.on('submit', 'form.activation-form', function (e) {
				e.preventDefault();
				var $this = $(this),
					panelWrap = $this.parents('.kwt-panel');
				panelWrap.addClass('kwt-loader');
				$.ajax({
					dataType: 'json',
					url: paWT.ajxUrl,
					data: {
					...
					success: function ( response ) {
						if( response ) {
							panelWrap.find('.kwt-messages').html( response.message );
							panelWrap.removeClass('kwt-loader');
		...
		pluginManager: function() {
			...
			$body.on('click', '.kwt-plugin-action', function (e) {
				e.preventDefault();
				var $this = $(this);
				current_item = $this.data('slug');
				$current_node = $this.parents('.plugin-item');
				process_current();
				$this.addClass('updating-message').removeClass('kwt-plugin-action');
			...
			function ajax_callback(response){
				if(typeof response == 'object' && typeof response.message != 'undefined') {
					...
					$('.kwt-install-core-plugins').html(response.message.message);
					if(typeof response.url != 'undefined'){
						// we have an ajax url action to perform.
						if(response.hash == current_item_hash){
							$current_node.find('span').html('<span class="kwt-msg kwt-error">'+ wp.updates.l10n.installFailedShort +'</span>');
						}else {
							...
							jQuery.post(response.url, response, function(response2) {
								jQuery.post( paWT.ajxUrl, {
									...
									$current_node.find('.kwt-process-button').replaceWith( res_process.process_button );
								} ).fail(ajax_callback);
							}).fail(ajax_callback);
						}
					}
				} else {
					$current_node.find('span').html('<span class="kwt-msg kwt-success">Succcess</span>');
				}
			}
...
killarMediaInit = wtadmin.initialization.mediaInit;