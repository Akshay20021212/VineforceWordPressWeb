...
class KillarWT_Import_Export {
	...
	public function admin_menu() {
		...
		add_submenu_page( 'killar-customizer-import-export', 'Killar Cutomizer Import / Export', 'Customizer Import / Export', 'manage_options', 'killar-customizer-import-export', array( $this, 'customizer_import_export_page' ), 5  );
		
	}
	
	public function customizer_import_export_page() {
		require_once KILLARWT_ADMIN . '/views/header.php';
		require_once KILLARWT_ADMIN . '/views/customizer-import-export.php';
	...
	static private function _import()
	{
		...
		if ( ! file_exists( $file['file'] ) ) {
			$cie_error = __( 'Error importing settings! Please try again.', 'killar' );
			return;
		}
		...
		if ( 'array' != gettype( $data ) ) {
			$cie_error = __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'killar' );
			return;
		}
		...
		if ( $data['template'] != $template ) {
			$cie_error = __( 'Error importing settings! The settings you uploaded are not for the current theme.', 'killar' );
			return;
		}
		...
		if ( ! empty( $data['mods'] ) ) {
			...
			// Success message
			$cie_success  = esc_attr__( 'Settings imported successfully.', 'killar' );
		}
		else {
			$cie_error = esc_attr__( 'No import data found.', 'killar' );
		}
	}
}

new KillarWT_Import_Export();