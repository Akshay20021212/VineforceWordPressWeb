<?php
/*
Element Description: Scene
*/

if ( ! function_exists( 'killarwt_scene' ) ) :

	function killarwt_scene( $atts = array() ) {
		...
		$css_output					= '';
		$args						= $atts;
		$args['id']  				= killarwt_uniqid('killar-scene-');
		$args['wrap_atts'] 			= array();
		$args['wrap_style_css'] 	= array();
		$args['sec_content'] 	    = '';
		$args['wrap_atts']['id'] 		= $args['id'];
		$args['wrap_atts']['class'] 	= array( 'killar-element', 'killar-scene' );
...
add_shortcode( 'killar_scene', 'killarwt_scene' );