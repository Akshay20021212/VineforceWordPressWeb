( function() {
	...
	wp.customize.bind( 'pane-contents-reflowed', function() {
		...
		wp.customize.panel.each( function( panel ) {
			if ( 'killar-nested' !== panel.params.type || _.isUndefined( panel.params.panel ) ) {
				return;
			}
			panels.push( panel );
		} );
	...
	wp.customize.Panel = wp.customize.Panel.extend( {
		attachEvents: function() {
			...
			if ( 'killar-nested' !== this.params.type || _.isUndefined( this.params.panel ) ) {
		...
		isContextuallyActive: function() {
			...
			if ( 'killar-nested' !== this.params.type ) {
