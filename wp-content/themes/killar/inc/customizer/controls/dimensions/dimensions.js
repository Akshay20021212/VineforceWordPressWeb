...
/* global dimensionskillarL10n */
wp.customize.controlConstructor['killar-dimensions'] = wp.customize.killarwtBaseControl.extend( {
	 ...
	 * @returns {void}
	 */
	killarwtNotifications: function() {
			...
			setting.bind( function( value ) {
				...
				if ( false === control.validateCssValue( value ) && ( ! acceptUnitless || isNaN( value ) ) ) {
					setting.notifications.add( code, new wp.customize.Notification( code, {
						type: 'warning',
						message: dimensionskillarL10n['invalid-value']
					} ) );
				} else {
...
jQuery( document ).ready( function($) {
	...
	// Connected button
	$('body').on('click', '.killarwt-connected' , function() {
	...
	// Connected button
	$('body').on('click', '.killarwt-disconnected' , function() {
