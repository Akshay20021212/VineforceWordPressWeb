 ...
 * @return void
 */
function killarwt_breadcrumb_trail( $args = array() ) {

	// Return if breadcrumbs are disabled
	if ( ! killarwt_page_title_breadcrumbs_display()
		|| is_front_page() ) {
		return;
	}
	...
	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new KillarWT_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class KillarWT_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `KillarWT_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see KillarWT_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see KillarWT_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function get_trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'breadcrumb-item font--medium';
			...
			// Postion class
			$p_class = '';
			if ( '' != get_theme_mod( 'killar_breadcrumbs_position' ) ) {
				$p_class = ' position-' . get_theme_mod( 'killar_breadcrumbs_position' );
			}
	...
	public function trail() {
		...
		echo apply_filters( 'killar_breadcrumb_trail', $this->get_trail() );
	}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'killar' ),
			'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'killar' ),
			'home'           		=> esc_html__( 'Home', 'killar' ),
			'error_404'           	=> esc_html__( '404 Not Found', 'killar' ),
			'archives'            	=> esc_html__( 'Archives',                              'killar' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'           		=> esc_html__( 'Search results for', 'killar' ),
			// Translators: %s is the page number.
			'paged'               	=> esc_html__( 'Page %s',                               'killar' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'killar' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      	=> esc_html__( 'Minute %s',                             'killar' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        	=> esc_html__( 'Week %s',                               'killar' ),
	...
	protected function add_network_home_link() {
		...
		$home_style = get_theme_mod( 'killar_page_title_breadcrumb_home_style', 'text' );
		if ( $home_style == 'icon' ) {
	...
	protected function add_singular_items() {
		...
		// Post type taxonomy
		$posts_tax 		= get_theme_mod( 'killar_breadcrumb_posts_taxonomy', 'category' );
		$products_tax 	= get_theme_mod( 'killar_breadcrumb_products_taxonomy', 'shop' );
		$portfolio_tax 	= get_theme_mod( 'killar_breadcrumb_portfolio_taxonomy', 'killar_portfolio_category' );
		...
		// If the post type is 'killar_portfolio'.
		else if ( class_exists( 'Killar_Portfolio' )
			&& 'killar_portfolio' === $post->post_type
			&& 'none' != $portfolio_tax ) {
		...
		if (get_theme_mod( 'killar_page_title_breadcrumbs_item_title_display', true ) == true
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'killar' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'killar' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'killar' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'killar' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'killar' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'killar' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'killar' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'killar' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'killar' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'killar' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'killar' ) ) ) );
